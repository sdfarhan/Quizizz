namespace Quizizz.Services.Events
{
    ...
    public class EventsService : IEventsService
    {
            ...
            string searchText = null)
        {
            ...
            query = query.Where(x => x.EventsGroups.Any(x => x.Group.StudentsGroups.Any(x => x.StudentId == studentId)));
        ...
        public async Task SendEmailsToEventGroups(string eventId, string emailHtmlContent)
        {
            var eventInfo = await this.repository
                ...
                .Select(x => new
                {
                    ...
                    Emails = x.EventsGroups.SelectMany(x => x.Group.StudentsGroups.Select(x => x.Student.Email)),
                }).FirstOrDefaultAsync();
        ...
        public async Task<int> GetEventsCountByStudentIdAndStatusAsync(string id, Status status, string searchCriteria = null, string searchText = null)
        {
            var query = this.repository
                            .AllAsNoTracking()
                            .Where(x => x.EventsGroups.Any(x => x.Group.StudentsGroups.Any(x => x.StudentId == id)))
                            .Where(x => x.Status == status);
